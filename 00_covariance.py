...
for total_user in total_users:
  ...
  for sample in range(sample_size):
    ...
    G_sample.append(channel_BS_IRS)
    ...
    Rg_sample.append(channel_covariance_irs)
  ...
  Rg_sample = np.array(Rg_sample)
  print(f'Rg_sample.shape: {Rg_sample.shape}')
...
#   # shuffle the idx
  ...
  # save the channel between BS and RIS
  G_train = G_sample[idx[:train_size]]
  G_test = G_sample[idx[train_size:]]
  
  print(f'G_train.shape: {G_train.shape}')
  print(f'G_test.shape: {G_test.shape}')
  
  # save the covariance between RIS and users
  Rg_train = Rg_sample[idx[:train_size]]
  Rg_test = Rg_sample[idx[train_size:]]
  
  print(f'Rg_train.shape: {Rg_train.shape}')
  print(f'Rg_test.shape: {Rg_test.shape}')
  ...
  # Now, save the data.
  np.save(f'train/{total_user}users/G_train.npy', G_train)
  np.save(f'test/{total_user}users/G_test.npy', G_test) 
  np.save(f'train/{total_user}users/Rg_train.npy', Rg_train)
  np.save(f'test/{total_user}users/Rg_test.npy', Rg_test)
  ...
  rateZ = []
  _, _, _, _, NoiseVarTotal, _  = data_preparation(cov_test)
  for snr in range(-10, 25, 5):
    ...
    sumRateZ = np.mean(compute_sum_rate(beam_test, cov_test, scaledFactor))
    rateZ.append(sumRateZ)
    
  ensure_dir(f'Plotting/{total_user}users/')
  np.save(f'Plotting/{total_user}users/sumRateZF.npy', np.array(rateZ))
