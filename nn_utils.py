...
# ---------------------------------------------------------------------
# --- Utility functions for the unsupervised model beam part --
# ---------------------------------------------------------------------
...
def plot_line(data, label, line_style, color, marker):
  plt.plot(range(-10, 25, 5), data, label=label, linestyle=line_style, color=color, marker=marker, markersize=7)
  
# ---------------------------------------------------------------------
# --- Utility functions for the supervised model RIS part --
# ---------------------------------------------------------------------
  
def reshape_phi(temp):
  K, N, phi_pred, phi_true = temp
  batch_shape = tf.shape(phi_true)[0]
  phi_pred = tf.cast(phi_pred, dtype=tf.float32)
  phi_pred = tf.reshape(phi_pred, [batch_shape, K, N, 1])
  return phi_pred

def loss_phi(temp):
    phi_true, phi_pred, N = temp
    N_casted = tf.cast(N, dtype=tf.float32)
    phi_diff = tf.subtract(phi_true, phi_pred)
    phi_diff_norm = tf.linalg.norm(phi_diff, ord='euclidean', axis=[2, 3])
    loss = (1.0 / tf.sqrt(N_casted)) * tf.reduce_sum(phi_diff_norm, axis=1)
    # Compute the mean loss across samples and users
    loss = tf.reduce_mean(loss)
    return loss

def loss_curve_phi(history, user_size, type):
  plt.figure(figsize=(8, 6))
  plt.plot(history.history['loss'], label='Training Loss')
  plt.plot(history.history['val_loss'], label='Validation Loss')
  title = f'Training and Validation Loss over Epochs for {user_size} Users of {type} NN model'
  plt.title("\n".join(wrap(title, 60))) # Wrap title at 60 characters
  plt.xlabel('Epoch')
  plt.ylabel('Loss')
  plt.legend()
