...
for user_size in total_users:
  ...
  with Timer() as timer:
    ...
    covariance_complex = np.load(f'test/{user_size}users/cov_test.npy')[:100, :, :, :]
    e_max_complex = np.load(f'test/{user_size}users/eMax_test.npy')[:100, :, :, :]
  ...
  np.save(f'test/{user_size}users/timeArraySuper_plot_7.npy', np.array(time_list))
  np.save(f'Plotting/{user_size}users/sumRateSuper_plot_7.npy', np.array(rate))
  print("Done!")
